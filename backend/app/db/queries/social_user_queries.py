...
async def get_social_user(
    ...
    FROM 
        public.user_social AS us
    JOIN 
        public.app_user AS u  
    ON us.user_id = u.id
    LEFT JOIN 
        public.user_info AS ui 
    ON 
        ui.user_id = u.id
    WHERE 
        us.provider = $1
...
async def create_social_user(
    ...
    async with AsyncDBPool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                """
                INSERT INTO 
                    public.user_info (user_id, email, nickname)
                VALUES 
                    ($1, $2, $3)
                """,
            ...
            await conn.execute(
                """
                INSERT INTO 
                    public.user_social (user_id, provider, provider_id)
                VALUES 
                    ($1, $2, $3)
                """,
